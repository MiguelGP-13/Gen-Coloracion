\section{Coloración con modelos de difusión}
...
En el contexto de la coloración, los modelos de difusión resultan útiles porque permiten generar detalles cromáticos plausibles a partir de información parcial (imágenes en escala de grises), preservando bordes y texturas.
...
la tarea de coloración, especializandose desde el inicio.
...
y adaptándolo específicamente a la tarea de coloración.Por otro lado, esta estrategia es la más costosa computacionalmente
...
Ha resultado ser más importante el color scale en el resultado que el número de 
pasos. En nuestros experimentos, se ha definido 1.5 como el mejor color scale.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]{images/color_scale_comparacion.png}
    \caption{Comparación hiperparámetros en imagen de coche}
    \label{fig:Comparacion_coche}
\end{figure}
\FloatBarrier

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]{images/color_scale_comparacion_perro.png}
    \caption{Comparación hiperparámetros en imagen de un perro}
    \label{fig:Comparacion_perro}
\end{figure}
\FloatBarrier

\subsection{Resultados}

Para la evaluación cuantitativa de la coloración de imágenes se han considerado tres métricas
...
la diferencia de color teniendo en cuenta la percepción humana; en el contexto de la coloración
...
\begin{table}[htbp]
    \centering
    \caption{Resultados de coloración con modelos de difusión.}
...
las imágenes sin deformaciones, como refleja el bajo valor de \texttt{LPIPS}. No obstante, en la 
tarea de coloración se evidencian limitaciones, ya que los colores generados difieren de los tonos 
de referencia. La métrica \texttt{CIEDE2000} permite cuantificar estas diferencias cromáticas desde 
una perspectiva perceptual: valores superiores a 3 ya se consideran inaceptables en aplicaciones de 
control de calidad, mientras que diferencias por encima de 10 indican una discrepancia claramente 
notable para el observador humano \cite{luo2001ciede2000}. En las imágenes obtenidas se aprecia que, 
aunque los modelos logran colorear las regiones principales, tienden a equivocarse en el tono o a 
producir colores más apagados de lo esperado. Entre las estrategias evaluadas, el \textit{fine-tuning} 
del último bloque ofrece el peor desempeño, mientras que la variante LoRA consigue los mejores 
...
\begin{itemize}
    ...
    \item Las coloraciones presentan regiones con aspecto grisáceo o, en ocasiones, fogonazos de color incoherentes.
...
No se ha logrado un modelo de difusión capaz de colorizar imagenes a un bune nivel.
Los modelos del artículo Diffusing Colors \cite{Zabari2023Diffusing} logran mejores resultados 
porque se apoyan en modelos de difusión previamente entrenados a gran escala, 
integran prompts textuales para guiar la coloración y aprovechan datasets masivos y diversos.

Viendo estas limitaciones, vamos a intentar atacar la única que podemos intentar 
resolver, integran prompts textuales para guiar la coloración.